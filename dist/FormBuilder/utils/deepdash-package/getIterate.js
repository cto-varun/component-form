...
var _getPathToString = _interopRequireDefault(require("./getPathToString.js"));
...
function getIterate(_) {
  var pathToString = (0, _getPathToString.default)(_);
  function iterate(item) {
    ...
    while (item) {
      ...
      if (item.needCallback && options.callbackAfterIterate) {
        ...
        try {
          callback(item.value, item.key, item.parent && item.parent.value, contextReader$1);
        } catch (err) {
          if (err.message) {
...
function describeValue(value, ownPropertiesOnly) {
  var res = {
    isObject: (0, _isObject.default)(value)
  };
