...
import { Form, Tooltip, Input, Alert } from 'antd';
...
function FormBuilderField(props) {
        ...
        className: `${
            meta.viewMode
                ? `ant-form-item-view-mode${
                      isV4 ? ' ant-form-item-view-mode-v4' : ''
                  }`
                : ''
        } ${
            field.className ||
            (field.formItemProps && field.formItemProps.className)
        }`,
    };
    if (isV4) {
    ...
    if (field.colSpan && formItemProps.labelCol && !field.formItemLayout) {
        const labelCol = Math.round(
            formItemProps.labelCol.span / field.colSpan
        );
        Object.assign(formItemProps, {
    ...
    if (isFieldViewMode) {
        ...
        if (field.renderView) {
            ...
            const href =
                typeof field.link === 'string' ? field.link : viewValue;
            viewEle = (
        ...
        if (form && field.readOnly) {
            const ele = (
                <span className="antd-form-builder-read-only-content">
                    {viewEle}
                </span>
            );
            return (
